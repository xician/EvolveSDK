...
-(ContactNode *)generateNode{
    ...
    [btnMaterial.diffuse setContents:[UIImage imageNamed:@"contactSaveBG.png"]];
...
//    [self setEulerAngles:SCNVector3Make((M_PI/-2)+[self.transf rotationX], 0-[self.transf rotationY], 0+[self.transf rotationZ])];
    ...
    /// Add text to button
    labelNode = [SCNText textWithString:[NSString stringWithFormat:@"%@ %@",[self.action.paramDictionary objectForKey:@"firstname"],[self.action.paramDictionary objectForKey:@"lastname"]] extrusionDepth:0.1];
...
-(void)resizeCanvasScene:(CGSize)size{
    ...
    if(noteViewedYet){
        ...
        [param setObject:NODE_TYPE_Contact forKey:EVENT_ASSET_TYPE];
