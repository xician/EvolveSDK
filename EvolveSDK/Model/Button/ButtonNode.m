...
//    [self setEulerAngles:SCNVector3Make((M_PI/-2)+[self.transf rotationX], 0-[self.transf rotationY], 0+[self.transf rotationZ])];
    ...
    /// Add text to button
    labelNode = [SCNText textWithString:[NSString stringWithFormat:@"%@",[self.appearance title]] extrusionDepth:0.1];
    
    SCNMaterial *frontMaterial = [SCNMaterial material];
    [frontMaterial.diffuse setContents:[self.appearance textColor]];
    SCNMaterial *backMaterial = [SCNMaterial material];
    backMaterial.diffuse.contents = [self.appearance textColor];
    SCNMaterial *sideMaterial = [SCNMaterial material];
    sideMaterial.diffuse.contents = [self.appearance textColor];
    ...
    [self.textNode setScale:SCNVector3Make(0.724007808/widthOfTextNode,0.022,0.07)];
...
//    [self.textNode setScale:SCNVector3Make(wx,hx,0.005)];
    ...
    if(noteViewedYet){
        ...
        [param setObject:NODE_TYPE_Button forKey:EVENT_ASSET_TYPE];
